...
\else
	...
	\pgfmathsetmacro\radius{\paperwidth*0.40/\ptsPerMm}
\fi
...
\foreach \spokeIndex [evaluate={\spoke=(\spokeIndex*2*\angle)}] in {0,...,\nSpokes}
{
	...
	\filldraw [fill=black, draw=black]
		(current page.center)
		-- +({\radius*cos(\start)}, {\radius*sin(\start)})
		-- +({\radius*cos(\stop)}, {\radius*sin(\stop)})
		-- cycle;
};
% MAGIC experimental circle radius, depends on your printer
\filldraw[white] (current page.center) circle (0.5pt);
\end{tikzpicture}%
